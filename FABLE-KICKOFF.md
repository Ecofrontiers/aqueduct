...
- Real-vs-sim ledger page (`/ledger`).
...
1. **The app IS Aqueduct — that's why we forked it.** Not "a mode of the Atlas."
   Header rebrands to an AQUEDUCT wordmark set in the Atlas's own design language
   (DM Sans, same light header). Nav: Map / Ledger / About. Regen Atlas credited in
   About + README. `/` is the Aqueduct map view because everything is Aqueduct.
...
1. **One app.** Aqueduct is a mode of the Atlas — its own route, and the landing view
   ...
   approach to everything. **The exit test: screenshot an Atlas page and an Aqueduct
   view side by side — if a stranger would say "two apps," it is not done.**
2. **The map is the content.** The Aqueduct view opens framed on Chiapas: lot markers,
   venue nodes, storage nodes, intent/route arcs as first-class map content. Atlas's
   bioregion browse stays fully functional via its normal nav, with zero Aqueduct
...
| Venue stance | Neutral rails venues join; Aqueduct adds no toll; incumbents render as nodes, never competitors |
...
Walk the current Aqueduct build. Read `atlas/src/aqueduct/`. Do not write code until
you can say what the Atlas's design system is in one paragraph.

**Phase 1 — Integration rebuild.** The Aqueduct route as landing view per the corrected
direction: Atlas-styled feed docked in the layout, tour rail in the view, restyle every
Aqueduct component to the Atlas system, fix the feed empty-state bug, camera framed on
