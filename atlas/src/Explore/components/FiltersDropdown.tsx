  ...
  return (
    <>
      ...
      >
        {openFilter === "assetType" && (
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 pb-2.5 mb-2.5 border-b border-gray-200">
              ...
              {investableOnly !== undefined && onToggleInvestableOnly && (
                <button
                  type="button"
                  onClick={onToggleInvestableOnly}
                  className={clsx(
                    "group flex items-center gap-1.5 rounded-full py-0.5 px-1 pr-2 border cursor-pointer transition-colors text-left",
                    investableOnly ? "border-green-400 bg-green-50" : "border-gray-200 hover:border-green-400",
                  )}
                  title="Only show assets with a real financial-instrument flag (prefinancing/pretoken/yield-bearing) and orgs with a real treasury"
                >
                  <div
                    className={clsx(
                      "rounded-full w-4 h-4 flex items-center justify-center shrink-0",
                      investableOnly ? "bg-green-400" : "bg-gray-300",
                    )}
                  >
                    <Check size={10} className="text-white" />
                  </div>
                  <span className="text-xs text-gray-700">Investable only</span>
                </button>
              )}
                ...
                return (
                  <div key={assetType.id}>
                      ...
                      onClick={() => {
                        if (
                          ...
                          !assetType.asset_subtypes.find(
                            (item) => item?.total_asset_count && item?.total_asset_count > 0,
                          )
                        ) {
                          return;
                        }
                        if (
                          ...
                          !assetType.asset_subtypes.find((subtype) =>
                            subtype.issuer_counts?.find((issuer) => filters.providers.includes(issuer.issuer_id)),
                          )
                        ) {
                          return;
                        }
                        if (!selected) {
                          dispatchFilters({
                            ...
                            payload: {
                              id: assetType.id,
                              name: assetType.name,
                              subtypes: assetType.asset_subtypes.map((subtype) => subtype.id),
                            },
                          });
                        } else {
                          dispatchFilters({
                    ...
                    {assetType.asset_subtypes.map((subtype) => (
                      <div
                        key={subtype.id}
                        className={clsx("flex items-start gap-1.5", "ml-3 mb-1", "cursor-pointer hover:text-gray-600")}
                        onClick={() => {
                          if (filters.providers.length === 0 && subtype.total_asset_count === 0) {
                            return;
                          }
                          if (
                            filters.providers.length > 0 &&
                      ...
                      >
                        <CheckboxBox
                          ...
                          checked={filters.assetTypes[assetType.id]?.subtypes.includes(subtype.id)}
                        />
                        <span
                          className={clsx(
                            "text-xs leading-snug",
                            filters.providers.length === 0 && subtype.total_asset_count === 0 && "text-gray-400",
                            filters.providers.length > 0 &&
                              !subtype.issuer_counts?.find((issuer) => filters.providers.includes(issuer.issuer_id)) &&
                              "text-gray-400",
                          )}
        ...
        {openFilter === "issuers" &&
          (() => {
            const searched = q ? base.issuers.filter((p) => p.name.toLowerCase().includes(q)) : base.issuers;
                ...
                <div className="grid grid-cols-4 gap-x-3 gap-y-0.5">
                  {filtered.map((provider) => {
                    ...
                    return (
                      ...
                      >
                        <CheckboxBox variant="small" className="!border-gray-400 flex-shrink-0" checked={selected} />
        ...
        {openFilter === "platforms" &&
          (() => {
            const searched = q ? base.platforms.filter((p) => p.name.toLowerCase().includes(q)) : base.platforms;
            const filtered = [...searched].sort((a, b) =>
              sortMode === "count"
                  ...
                  {filtered.map((plat) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={clsx("text-xs truncate", selected ? "text-gray-900 font-medium" : "text-gray-700")}
                        >
                          {plat.name}
                        </span>
                        <span className="text-[10px] text-gray-400 ml-auto shrink-0">{count}</span>
