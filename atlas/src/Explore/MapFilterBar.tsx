import { Cube, Lightning, Users, X } from "@phosphor-icons/react";
import clsx from "clsx";
import { useMemo, useState } from "react";
import { useNewFiltersDispatch, useNewFiltersState } from "../context/filters";
import type { ActorTypeKey, EntityTypeKey } from "../context/filters/filtersContext";
...
export function MapFilterBar({
  ...
  const [selectedFilter, setSelectedFilter] = useState<"assetType" | "issuers" | "platforms">("assetType");
  ...
  const availableSdgs = useMemo(() => {
    ...
    return Array.from(map.entries()).sort((a, b) => Number.parseInt(a[0], 10) - Number.parseInt(b[0], 10));
  }, [allActions]);
    ...
    <div className="hidden md:block absolute top-0 left-0 right-0 z-10">
      <div className="bg-gray-900/90 backdrop-blur-sm flex items-center px-2 h-8">
        ...
        <div className="flex items-center h-full">
          <button
            ...
            style={
              activeEntityTypes.has("asset") ? { borderBottom: `2px solid ${ENTITY_COLORS.asset.primary}` } : undefined
            }
          >
            Assets
          </button>
                ...
                [
                  {
                    ...
                    label: "Type",
                    hasFilter: hasTypeFilter,
                    count: accumulateSubtypes().length,
                    reset: "RESET_TYPE_FILTERS",
                  },
                  {
                    key: "issuers" as const,
                    label: "Issuer",
                    hasFilter: hasIssuerFilter,
                    count: filters.providers.length,
                    reset: "RESET_PROVIDER_FILTER",
                  },
                  {
                    key: "platforms" as const,
                    label: "Chain",
                    hasFilter: hasChainFilter,
                    count: filters.platforms.length,
                    reset: "RESET_PLATFORM_FILTER",
                  },
                ] as const
              ).map((f, i) => (
                <div key={f.key} className="flex items-center h-full">
                  ...
                  <button
                    onClick={() => {
                      ...
                      setIsDropdownOpen(selectedFilter === f.key ? !isDropdownOpen : true);
                    }}
                    className={clsx(
                      "h-full flex items-center gap-1 px-1.5 text-[11px] transition-colors cursor-pointer",
                      isDropdownOpen && selectedFilter === f.key ? "text-white" : "text-white/40 hover:text-white/70",
                    )}
                  >
          ...
          <button
            ...
            style={
              activeEntityTypes.has("action")
                ? { borderBottom: `2px solid ${ENTITY_COLORS.action.primary}` }
                : undefined
            }
          >
            Actions
        ...
        <div className="flex items-center h-full">
          <button
            ...
            style={
              activeEntityTypes.has("actor") ? { borderBottom: `2px solid ${ENTITY_COLORS.actor.primary}` } : undefined
            }
          >
            Actors
          </button>
                  ...
                  return (
                    <div key={toggle.key} className="flex items-center h-full">
                      ...
                      <button
                        ...
                        className={clsx(
                          "h-full flex items-center px-1.5 text-[10px] font-medium transition-colors cursor-pointer",
        ...
        <div className="flex items-center gap-2 px-2 py-1">
          ...
          {activeEntityTypes.has("asset") && activeEntityTypes.has("action") && (
          ...
          {(activeEntityTypes.has("asset") || activeEntityTypes.has("action")) && activeEntityTypes.has("actor") && (
            <div className="w-px h-3 bg-white/15" />
          )}
      ...
      {isDropdownOpen && (
        <FiltersDropdown
          ...
          onToggleInvestableOnly={onToggleInvestableOnly}
        />
      )}
            ...
            {ALL_SDGS.map(({ code, title }) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={clsx(
                      "flex-1 text-xs truncate group-hover:whitespace-normal group-hover:overflow-visible",
      ...
      {actionDropdown === "time" &&
        ...
        (() => {
          ...
          const toIdx = actionFilters?.timeRange
            ? Math.max(0, timeRange.indexOf(actionFilters.timeRange.to))
            : timeRange.length - 1;
                  ...
                  const onMove = (ev: MouseEvent) => {
                    ...
                    if (dragging === "from") {
                      ...
                      if (clamped >= 0)
                        onActionFiltersChange?.({
                          ...actionFilters!,
                          timeRange: { from: timeRange[clamped], to: timeRange[toIdx] },
                        });
                    } else {
                      const clamped = Math.max(idx, fromIdx + 1);
                      if (clamped <= timeRange.length - 1)
                        onActionFiltersChange?.({
                          ...actionFilters!,
                          timeRange: { from: timeRange[fromIdx], to: timeRange[clamped] },
                        });
                    }
                  };
                  const onUp = () => {
