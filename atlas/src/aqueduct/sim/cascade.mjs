...
export async function buildCascade(lot) {
  ...
  const lotIdShort = (lot.aqueduct_id || "aq:").replace("aq:", "aq:").slice(0, 10) + "…" + (lot.aqueduct_id_full || "").slice(-3);

  // ---- Chapter 1: Aggregate (B1) ----
  const scoutReadId = push({
    chapter: "aggregate", beat: "B1", prov: "LIVE", agent: "@scout-ethichub",
    verb: "read", object: `${shopLotsTotal} lots at greencoffee.ethichub.com`, detail: `${chiapasMatched} matched Chiapas`,
    valueText: "1.8s", status: "OK",
    expand: {
  ...
  const pinId = push({
    chapter: "aggregate", beat: "B1", prov: lot.provenance === "LIVE" ? "LIVE" : "SNAPSHOT", agent: "@scout-ethichub",
    verb: "pinned", object: `lot ${lot.aqueduct_id}`, detail: `${lot.producer.initials} / ${lot.origin.region} (${lot.origin.country}) – ${lot.variety} ${lot.process} – ${lot.quality.sca_score} SCA`,
    valueText: lot.price ? `€${lot.price.amount.toFixed(2)}/kg` : "—", status: "OK",
  ...
  if (lot.lending?.matched) {
    identityId = push({
      chapter: "verify", beat: "B2", prov: "LIVE", agent: "@diligence-identity",
      verb: "matched", object: `${lot.lending.projects.length} lending project(s)`, detail: `join_confidence: ${lot.join_confidence}`,
      valueText: "cross-surface", status: "PARTIAL",
      parentId: pinId,
      expand: {
        headline: "Two EthicHub surfaces — the public shop lot and the lending-project API — resolved into one entity, linked by producer/community, not by platform id.",
        sections: [
          ...
          { label: "labeling", value: "cross-surface (both surfaces EthicHub's) — promoted to cross-platform only when a second connector lands" },
        ],
      },
    });
  }

  const eudrChecked = [lot.eudr?.plot_geo_present, lot.eudr?.harvest_window_present, lot.eudr?.legality_evidence, Boolean(lot.eudr?.dds_ref)];
  const eudrVerifiedCount = eudrChecked.filter(Boolean).length;
  const eudrId = push({
    chapter: "verify", beat: "B3", prov: "LIVE", agent: "@diligence-eudr",
    verb: "checked", object: `6 EUDR fields on ${lot.aqueduct_id}`, detail: `${eudrVerifiedCount * 1} of 4 concrete fields verified, ${4 - eudrVerifiedCount} unverifiable`,
    valueText: "3.2s", status: "PARTIAL",
    parentId: identityId ?? pinId,
    expand: {
      headline: "The diligence agent independently re-checked the EUDR document chain on the anchor lot — plot geolocation and a Due Diligence Statement reference are honestly missing, not badged green.",
      sections: [
  ...
  const intentId = push({
    chapter: "publish", beat: "B5", prov: "TESTNET", agent: "@intent-registry",
    verb: "published", object: `sell-this-lot intent aq:i-04 for ${lot.aqueduct_id}`, detail: `ceiling ≤ €${landedCeilingPreview?.toFixed(2) ?? "—"}/kg landed`,
    valueText: lot.price ? `€${lot.price.amount.toFixed(2)}/kg FOB` : "—", status: "OK",
    parentId: priceId,
    expand: {
      headline: "A sell-this-lot intent published against the deployed Base Sepolia IntentRegistry — solvers can now compete to fill it.",
      sections: [
  ...
  const financeId = push({
    chapter: "publish", beat: "B6", prov: "SIM", agent: "@intent-registry",
    verb: "published", object: `finance-this-planting intent ${financeIntent.id}`, detail: `${financeIntent.seedlings.toLocaleString()} seedlings — ${financeIntent.community}`,
    valueText: `€${financeIntent.totalEur.toLocaleString()}`, status: "OK",
    parentId: intentId,
    expand: {
      headline: "Finance the system, sell its harvest: a second, parallel intent routes production financing to an agroforestry venue for the same origin community.",
      sections: [
  ...
  const fundedId = push({
    chapter: "publish", beat: "B6", prov: "SIM", agent: financeIntent.venue.handle,
    verb: "funded", object: `intent ${financeIntent.id}`, detail: `planting renovation, ${financeIntent.seedlings.toLocaleString()} seedlings`,
    valueText: `€${financeIntent.totalEur.toLocaleString()}`, status: "FILLED",
    parentId: financeId,
    expand: {
      ...
      sections: [{ label: "venue status", value: financeIntent.venue.status }, { label: "note", value: financeIntent.venue.note }],
    },
  });
  ...
  for (const b of race.bids) {
    ...
    const id = push({
      ...
      valueText: `€${b.bid.landedEurPerKg.toFixed(4)}/kg landed`, status: "BID", parentId: intentId,
      expand: {
  ...
  const winnerId = push({
    chapter: "fill", beat: "B7", prov: winner.real ? "LIVE" : "SIM", agent: winner.handle,
    verb: winner.real ? "undercut" : "undercut", object: "cleared the buyer ceiling", detail: `prev best €${race.bids.filter((b)=>b.bid&&b.handle!==winner.handle).sort((a,b)=>a.bid.landedEurPerKg-b.bid.landedEurPerKg)[0]?.bid.landedEurPerKg.toFixed(4) ?? "—"} → new €${winner.bid.landedEurPerKg.toFixed(4)}/kg`,
    valueText: `€${winner.bid.landedEurPerKg.toFixed(4)}/kg`, status: "UNDERCUT", parentId: prevBidParent,
  });

  const buyerMatch = evaluateBuyerMatch({ lot, winningBid: winner.bid });
  const matchId = push({
    chapter: "fill", beat: "B7", prov: "SIM", agent: SIM_BUYER_HANDLE(), verb: "matched", object: `intent aq:i-04`, detail: `${buyerMatch.rows.filter((r) => r.pass).length}/${buyerMatch.rows.length} criteria satisfied`,
    valueText: buyerMatch.matched ? "FILLED" : "no match", status: buyerMatch.matched ? "FILLED" : "DECLINED", parentId: winnerId,
    expand: {
      headline: `${buyerMatch.buyer.handle} — standing demand — matched: ${buyerMatch.rows.map((r) => `${r.label} ${r.pass ? "✓" : "✗"} ${r.got}`).join("; ")}.`,
      sections: buyerMatch.rows.map((r) => ({ label: r.label, value: `${r.pass ? "✓" : "✗"} ${r.got} (want ${r.want})` })),
    },
  });

  const x402Id = push({
    chapter: "fill", beat: "B7", prov: "SIM", agent: winner.handle,
    verb: "paid", object: "data fee via x402", detail: "1 call",
    valueText: "$0.0040 USDC", status: "OK", parentId: matchId,
    expand: {
      headline: "Agent payment rendered SIM in the swarm — the x402 rail itself is proven (a real $0.004 Apify scrape payment on the operator's agentic wallet), not wired live into this demo.",
  ...
  // ---- Chapter 6: Settle (B8) ----
  const settleId = push({
    chapter: "settle", beat: "B8", prov: "TESTNET", agent: "@settle-base",
    verb: "settling", object: "intent aq:i-04", detail: `credited: ${COOP_EXPORTER_NODE.name}`,
    valueText: "awaiting broadcast", status: "PENDING", parentId: x402Id,
    expand: {
      headline: "Settlement prepared against the deployed Base Sepolia IntentRegistry — the unsigned transaction is ready; no private key exists in this repo, so it is not broadcast here.",
      sections: [
  ...
  const passthroughId = push({
    chapter: "settle", beat: "B8", prov: "SIM", agent: "@settle-base",
    verb: "settled", object: "farmer pass-through", detail: "via co-op credit stack",
    valueText: "—", status: "OK", parentId: settleId,
    expand: { headline: "Farmer pass-through renders as a labeled downstream step off the co-op node — never an instant spot payment to a farmer's phone (spec §5 settle realism)." },
  });
  const vaultId = push({
    chapter: "settle", beat: "B8", prov: "SIM", agent: VAULT_NODE.handle,
    verb: "accumulated", object: "1 lot", detail: VAULT_NODE.note,
    valueText: lot.price ? `€${(lot.price.amount * 70).toFixed(0)}` : "—", status: "OK", parentId: passthroughId,
  });

  return { events, oracle, race, buyerMatch, financeIntent, winner, settleId, meta: { cadenceMs: CADENCE_MS, lotIdShort } };
}
