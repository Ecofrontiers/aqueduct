...
export default function AqueductCoopSeat(): React.ReactElement {
  ...
  const dupCheck = useMemo(
    () => (seat ? duplicateFinancingCheck(seat, allLots) : null),
    [seat, allLots]
  );
    ...
    return (
      <>
        ...
        <div className="main-container pt-[100px] px-6">
          <p>
            Coop not found. <Link to="/" className="underline">Back to the map</Link>
          </p>
        </div>
      </>
    );
          ...
          <div className="grid md:grid-cols-2 gap-4">
              ...
              {!race ? (
                ...
                <div className="space-y-1.5">
                  <p className="text-[11px] text-gray-500">
                    ...
                    {avgFob.toFixed(2)}/kg FOB) — every bid is the same deterministic landed-cost
                    computation, competition drives YOUR landed cost down:
                  </p>
                  {race.bids.map((b: { handle: string; status: string; bid: { landedEurPerKg: number } | null; real?: boolean }) => (
                    <div key={b.handle} className="flex items-center justify-between gap-2 bg-gray-50 px-3 py-1.5">
                      ...
                      <span className="flex items-center gap-1.5">
                        ...
                        {b.status === "DECLINED" ? (
                          ...
                          <span className={`text-[11px] aq-mono ${b.handle === race.winner?.handle ? "font-bold text-green-700" : "text-gray-600"}`}>
                  ...
                  {race.winner && (
                    <p className="text-[11px] text-gray-500 pt-1">
                      Best landed: <span className="font-semibold text-gray-800">€{race.winner.bid.landedEurPerKg.toFixed(3)}/kg</span> by{" "}
                      <span className="aq-mono">{race.winner.handle}</span> — your asking FOB holds; the
                      competition compresses the middle, not your price.
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="bg-cardBackground border border-gray-200 px-5 py-4">
              ...
              <p className="text-[11px] text-gray-400 leading-relaxed mb-2">
                Advance rate is a declared assumption, not a citation. Eligibility below is the real
                policy-engine verdict — the same (institution, rule, condition, effect) machinery the
                solver race uses.
              </p>
              <div className="space-y-1">
                {projection.eligibleActors.map((a: { handle: string; name: string; kind: string; matchedLotCount: number; matchedValueEur: number }) => (
                ...
                {projection.eligibleActors.length === 0 && (
                  <p className="text-[11px] text-gray-400">
                    No capital actor's conditions match these lots yet — structure closes gaps
                    (quality grading, EUDR fields), matches follow.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-cardBackground border border-gray-200 px-5 py-4">
              ...
              <p className="text-[11px] text-gray-600 leading-relaxed mb-2">{projection.stablecoinRail.realPrecedent}</p>
            ...
            <div className="bg-cardBackground border border-gray-200 px-5 py-4">
              ...
              <p className="text-[11px] text-gray-400 leading-relaxed mt-1">
                This is the lender-facing registry service — protecting your credit access is the
                sustainability story, not a toll on your lot.
              </p>
            </div>
          </div>
